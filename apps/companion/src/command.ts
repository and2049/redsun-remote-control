...
export function parseCommand(args: readonly string[]): Command {
  ...
  if ((args.length === 5 || (args.length === 6 && args[5] === "--backend")) && args[0] === "serve" && args[1] === "--origin" && args[2] && args[3] === "--port" && args[4]) {
    ...
    if (origin.protocol === "https:" && origin.origin === args[2] && !origin.username && !origin.password && /^\d+$/.test(args[4]) && port >= 1 && port <= 65535) {
      return { kind: "serve", origin: args[2], port, ...(args[5] === "--backend" ? { backend: true as const } : {}) }
    }
  }
