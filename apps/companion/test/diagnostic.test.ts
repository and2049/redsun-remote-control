...
test("diagnostic assets build without dependencies and enforce host, method and CSP boundaries", async () => {
  ...
  const page = handle(new Request(origin + "/diagnostic"))
  ...
  expect(handle(new Request("https://attacker.example/diagnostic")).status).toBe(403)
  expect(handle(new Request(origin + "/diagnostic", { method: "POST" })).status).toBe(405)
  expect(handle(new Request(origin)).status).toBe(404)
